function sysCall_init() 
    ...
    fakeShadow=true
    sonar = sim.getObject('./Proximity_sensor')
...
function getDistance(sensor,dist)
    local detected,distace
    detected,distance = sim.readProximitySensor(sensor)
    return distance
end    

function sysCall_sensing()
    dist = getDistance(sonar,distance)
end
...
function sysCall_actuation() 
    ...
    position = sim.getObjectPose(targetObj, ref)
    if(dist > 0.75)
    then
        pose = {position[1],-4.25 + sim.getSimulationTime()*0.1 ,position[3] -0.025*sim.getSimulationTimeStep()}
